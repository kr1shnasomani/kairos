...
| Field routes | **The mobile bottom tab bar is currently NOT rendered.** `BottomTabs` still exists in `app-shell.tsx` (~line 239), but its render site (~line 626) is commented out — "temporarily removed per user request (revisiting mobile UX later)", so mobile navigates via the hamburger sidebar. There is no `FieldBottomTabs`; that name is from an older revision. Field gating itself is live: `isField = role === "field_worker"` (`use-role.ts` `FIELD_ROLES`, `roleHome` → `/briefs`). Routes under `/field`: `deviation`, `elicitation`, `voice`. SW offline is prod-only; the IndexedDB write queue (`idb.ts`) is app-level and works in dev. |
