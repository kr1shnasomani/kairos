...
export function PhaseBadge() {
  ...
  return (
    <span className="inline-flex h-[20px] items-center rounded-full bg-[color-mix(in_srgb,var(--info)_14%,transparent)] px-2 text-[10px] font-semibold text-info">
      {label}
    </span>
  );
}
...
export function KpiCard({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "group flex w-full flex-col gap-1 rounded-xl border border-line bg-surface p-4 text-left transition-colors",
        onClick && "hover:border-accent/40 hover:bg-surface-2 cursor-pointer",
        !onClick && "cursor-default",
      )}
    >
      <span className="text-[11px] font-medium uppercase tracking-[0.08em] text-muted">{label}</span>
      <span className={cn("tabular text-[28px] font-semibold leading-none", valueColor)}>{value}</span>
          ...
          <li key={ev.id} className="relative">
            ...
            <div>
              <div className="flex flex-wrap items-baseline gap-2">
                <span className="text-[13px] font-semibold text-ink">{ev.label}</span>
                <time className="tabular text-[11px] text-muted">{ev.timestamp}</time>
                {ev.meta && (
                  <span className="text-[11px] text-muted">{ev.meta}</span>
                )}
              </div>
              {ev.description && (
                <p className="mt-0.5 text-[12.5px] leading-snug text-muted">{ev.description}</p>
              )}
            </div>
          </li>
        );
...
export function EvidenceLineage({
  ...
  return (
    <div className="rounded-lg border border-line bg-surface-2 text-[12.5px]">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
      ...
      {open && (
        <div className="border-t border-line px-3 py-3 space-y-3">
          {sources && sources.length > 0 && (
            <div>
              <p className="mb-1.5 text-[11px] font-bold uppercase tracking-widest text-muted">Sources</p>
          ...
          {auditEntries && auditEntries.length > 0 && (
            <div>
              <p className="mb-1.5 text-[11px] font-bold uppercase tracking-widest text-muted">Audit trail</p>
              <ul className="space-y-1">
                {auditEntries.map((e) => (
                  <li key={e.log_id} className="flex flex-wrap gap-2 text-[11.5px] text-muted">
...
export function ConfidenceMeter({ value }: { value: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="tabular text-[11px] font-semibold text-muted">{pct}%</span>
  ...
  return (
    ...
    >
      <div className="flex items-start gap-2.5">
        ...
        <div>
          <p className="text-[13px] font-semibold text-danger">Safety-critical query — sources returned directly</p>
          {reason && <p className="mt-0.5 text-[12.5px] text-muted">{reason}</p>}
        </div>
      </div>

      {sources && sources.length > 0 && (
        <div>
          <p className="mb-1.5 text-[11px] font-bold uppercase tracking-widest text-muted">Relevant sources</p>
