...
export default function MocDetailPage() {
  ...
  if (!moc) {
    return (
      <div className="mx-auto max-w-3xl px-5 py-16 text-center text-[13px] text-muted">
        Loading MoC…
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-3xl px-5 py-8 sm:px-8 sm:py-10">
      <Link href="/governance/moc" className="inline-flex items-center gap-1.5 text-[13px] text-muted hover:text-ink">
      ...
      <header className="mt-4 flex flex-wrap items-center gap-x-3 gap-y-2">
        <h1 className="tabular text-[22px] font-semibold text-accent">{moc.moc_id}</h1>
        ...
        <span className="tabular ml-auto text-[11.5px] text-muted">{relativeTime(moc.created_at)}</span>
      </header>
            ...
            <div key={tag} className="rounded-lg border border-line bg-surface-2 p-3">
              ...
              {s?.document_id && (
                <Link
                  ...
                  className="tabular mt-0.5 block text-[11px] text-accent hover:underline"
                >
                  {s.document_id}
                </Link>
              )}
      ...
      {moc.draft_content && (
        <section className="mt-5">
          ...
          <div className="mt-2.5 rounded-xl border border-line bg-surface p-4 text-[13px] leading-relaxed text-ink">
            {moc.draft_content}
          </div>
          ...
          <div className="mt-3 flex flex-col gap-2.5">
            ...
            <div className="flex gap-2">
              <button
                ...
                disabled={busy}
                className="inline-flex h-9 items-center rounded-lg bg-accent px-4 text-[12.5px] font-semibold text-on-accent transition-opacity hover:opacity-90 disabled:opacity-50"
              >
                {busy ? "Approving…" : "Approve MoC"}
              </button>
              <Link
                href="/governance/conflicts"
                className="inline-flex h-9 items-center rounded-lg border border-line px-4 text-[12.5px] font-semibold text-muted transition-colors hover:bg-surface hover:text-ink"
              >
                View conflict
              </Link>
            </div>
      ...
      {!isPending && (
        <div className="mt-5 flex items-center gap-2 rounded-xl border border-line bg-surface px-4 py-3">
          <span className={`inline-flex items-center gap-1.5 text-[13px] font-semibold ${moc.status === "approved" ? "text-verified" : "text-danger"}`}>
