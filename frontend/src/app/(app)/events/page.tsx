...
const CHART_TONES = [TONE_VAR.danger, TONE_VAR.caution, TONE_VAR.info, TONE_VAR.neutral];
...
const COLUMNS: TableColumn<EventRow>[] = [
  ...
  {
    key: "priority", label: "Priority", sortValue: (r) => r.priority,
...
export default function EventsPage() {
  ...
  const priorities = useMemo(() => Array.from(new Set(events.map((e) => e.priority))), [events]);
        ...
        {showDiscreteBars ? (
          <BarChart data={trend} margin={{ top: 4, right: 8, bottom: 0, left: 0 }}>
            ...
            {priorities.map((p, index) => (
              <Bar key={p} dataKey={p} name={label(p)} fill={CHART_TONES[index % CHART_TONES.length]} isAnimationActive={!reduced} />
            ))}
          </BarChart>
        ) : (
          <LineChart data={trend} margin={{ top: 4, right: 8, bottom: 0, left: 0 }}>
            ...
            <Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: 11, color: "var(--muted)" }} />
            {priorities.map((p, index) => (
              <Line key={p} type="monotone" dataKey={p} name={label(p)} stroke={CHART_TONES[index % CHART_TONES.length]} strokeWidth={2} dot={false} isAnimationActive={!reduced} />
            ))}
          </LineChart>
        )}
      </ChartContainer>
