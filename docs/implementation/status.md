...
**All 13 architecture layers are implemented.** Architecture conformance is **~88%**, measured by an
independent re-audit on 2026-08-17 that scored *reachability in the path the architecture specifies*
rather than *presence of the mechanism* — see [Conformance](#architecture--implementation-conformance).
The previous ~94% figure was self-assessed against the looser rubric and is superseded.
...
| 4 | ~~**Trust boundary: fail-open authz, unenforced reads, client-supplied tenancy**~~ — **DONE 2026-08-17** | Four defects, all in the same boundary. (a) `_ask_opa` returned `True` on *any* exception, so OPA being unreachable silently disabled authorization everywhere — now returns `self.debug`, i.e. fail-closed outside dev. (b) The middleware read the app role from the token's **top-level** `role`, which is Supabase's Postgres role `authenticated` and matches no rego role — every real token would have been denied had the layer ever run; `claims_to_user` now reads `user_metadata.role`, mirroring `dependencies.get_current_user`. (c) Enforcement was write-only, so `read_audit`/`read_compliance`/`read_governance` were dead rego rules and any authenticated role could read the audit trail directly from the API — `_READ_ACTION_MAP` now gates those three prefixes on GET/HEAD (**not** OPTIONS: this middleware is outermost, so gating the CORS preflight would 401 every cross-origin request), and the three actions were added to `_sensitive_actions` so the catch-all cannot re-grant them. (d) `site_id` was a client-supplied query param on `/assets` and both `/compliance` reads — `dependencies.site_scope` now derives it from the verified token, 403s a cross-site request, and treats a blank site as "nothing" rather than "every site". `INTERNAL_API_KEY` is recognised as a service principal so the fail-closed path does not lock out the Go connector. Backend role grants now match the frontend matrix in `use-role.ts`. 30 tests in `tests/test_authz_boundary.py`; rego decisions verified against a throwaway OPA. | done |
...
- **OPA enforces writes only** (POST/PUT/DELETE), so a GET to `/governance/*` returns 200 for the
  compliance role — reads are gated by UI visibility, not policy. Promoting a **non-existent**
  quarantine item returns **500 rather than 404**.
