...
- [x] **`tests.yml` restructured into two tiers.** `unit` runs **65 service-free tests with no secrets and no network** — green on every push and fork PR. `integration` runs the full suite against `--profile local-stores` and **skips with exit 0** unless `CI_SUPABASE_*` is set. Previously the workflow pointed a write-heavy suite (`make init-all` + create/purge) at cloud credentials, which would have corrupted the golden dataset on every push; that is why the secrets are named `CI_SUPABASE_*` and must come from a throwaway project. `setup-ci-secrets.sh` sets them.
